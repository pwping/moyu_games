	...
	factory: (require) => {
		...
		var GameController = class {
			...
			getSnapshot() {
				return {
					open: this.open,
					taskEndPending: this.taskEndPending
				};
			}
			subscribe(fn) {
		...
		var TaskStartStream = class {
			...
			onError;
			source;
			constructor(path, onTaskStart, onTaskEnd = () => {}, onError) {
				this.path = path;
...
`;
		...
		*/
		function clearGameKeys(game) {
			for (const key of GAME_KEYS[game]) try {
		...
		function useControllerSnapshot(controller) {
			const [snap, setSnap] = (0, react.useState)(() => {
				...
				return {
					open: s.open,
					taskEndPending: s.taskEndPending
				};
			});
		...
		*/
		function GameOverlay(props) {
			...
			const [toastTask, setToastTask] = (0, react.useState)(null);
			(0, react.useEffect)(() => {
			...
			const [activeGame, setActiveGame] = usePersistedState("activeGame", "puzzle");
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						activeGame === "puzzle" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SlidingPuzzle, {
							...
							open,
							defaultSize
						}) : activeGame === "sudoku" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SudokuBoard, {
							t,
							open
						}) : activeGame === "snake" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SnakeGame, {
							t,
							open
						}) : activeGame === "schulte" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SchulteGrid, {
							t,
							open,
							defaultSize
						}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MemoryGrid, {
							t,
							open
						}),
						toastTask !== null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "moyu-toast-wrap",
							role: "status",
							"aria-live": "polite",
							onClick: () => setToastTask(null),
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "moyu-toast-card",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("svg", {
									...
									"aria-hidden": "true",
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("path", {
										d: "M12 2L4 7v5c0 5 3.2 9.2 8 10 4.8-.8 8-5 8-10V7l-8-5z",
										fill: "none",
									...
									className: "moyu-toast-text",
									children: "任务执行完成，先去看一下完成效果吧"
								})]
							})
						}) : null
					]
		...
		function SlidingPuzzle({ t, open = true, defaultSize = 5 }) {
			const initialSize = clampSize$1(defaultSize);
			const [size, setSize] = usePersistedState("puzzle:size", initialSize);
			const [board, setBoard] = usePersistedState("puzzle:board", () => moveBlankToBottomRight(shuffledBoard(initialSize), initialSize));
			const [moves, setMoves] = usePersistedState("puzzle:moves", 0);
			const [elapsedMs, setElapsedMs] = usePersistedState("puzzle:elapsedMs", 0);
			const [running, setRunning] = usePersistedState("puzzle:running", false);
			const solved = isSolved(board);
			const paused = !open || !running || solved;
			(0, react.useEffect)(() => {
				if (paused) return;
				const id = window.setInterval(() => setElapsedMs((m) => m + 1e3), 1e3);
		...
		function SchulteGrid({ t, open = true, defaultSize = 5 }) {
			const initialSize = clampSize$1(defaultSize);
			const [size, setSize] = usePersistedState("schulte:size", initialSize);
			const [board, setBoard] = usePersistedState("schulte:board", () => createBoard$2(initialSize));
			const [elapsedMs, setElapsedMs] = usePersistedState("schulte:elapsedMs", 0);
			const [running, setRunning] = usePersistedState("schulte:running", false);
		...
		/** 9×9 数独棋盘。 */
		function SudokuBoard({ t, open = true }) {
			const [board, setBoard] = (0, react.useState)(() => {
				try {
					const raw = localStorage.getItem("moyu-games:sudoku:board");
					if (raw !== null) return {
						...JSON.parse(raw),
						correctOnce: /* @__PURE__ */ new Set(),
						wrongOnce: /* @__PURE__ */ new Set()
			...
			(0, react.useEffect)(() => {
				try {
					...
					localStorage.setItem("moyu-games:sudoku:board", JSON.stringify(serial));
				} catch {}
			}, [board]);
			const [elapsedMs, setElapsedMs] = usePersistedState("sudoku:elapsedMs", 0);
			const [running, setRunning] = usePersistedState("sudoku:running", false);
			const paused = !open || !running || board.completed;
			(0, react.useEffect)(() => {
				if (paused) return;
				const id = window.setInterval(() => setElapsedMs((m) => m + 1e3), 1e3);
		...
		function MemoryGrid(props) {
			...
			const [board, setBoard] = usePersistedState("memory:board", () => createBoard(3));
			const [elapsedMs, setElapsedMs] = usePersistedState("memory:elapsedMs", 0);
			const [running, setRunning] = usePersistedState("memory:running", false);
			...
			const onCell = (index) => {
				setBoard((b) => {
					...
					if (!b.started && nb.started) setRunning(true);
		...
		function SnakeGame({ t, open = true }) {
			const canvasRef = (0, react.useRef)(null);
			const SNAKE_KEY = "moyu-games:snake:state";
			const gameRef = (0, react.useRef)(null);
			if (gameRef.current === null) try {
				const raw = localStorage.getItem(SNAKE_KEY);
				gameRef.current = raw !== null ? JSON.parse(raw) : createGame();
			} catch {
			...
			const [score, setScore] = usePersistedState("snake:score", 0);
			const [gameOver, setGameOver] = usePersistedState("snake:gameOver", false);
			const [started, setStarted] = usePersistedState("snake:started", false);
			(0, react.useEffect)(() => {
				startedRef.current = started;
			}, [started]);
			const TICK_MS = 200;
			const persistSnake = (0, react.useCallback)(() => {
				try {
					localStorage.setItem(SNAKE_KEY, JSON.stringify(gameRef.current));
				} catch {}
			}, [SNAKE_KEY]);
			(0, react.useEffect)(() => {
				persistSnake();
